...
flags.DEFINE_integer("max_epoch", 25, "The number of max epoch")
...
class RnnlmModel(object):
	...
	def __init__(self, is_training, config, input_):
		...
		self.initial = tf.reshape(tf.stack(axis = 0, values = self._initial_state_single), 
			[config.num_layers, 2, 1, size], name = "test_initial_state")
		...
		state_placeholder = tf.placeholder(tf.float32, 
			[config.num_layers, 2, 1, size], name="test_state_in")
		...
		with tf.device("/cpu:0"):
			self.embedding = tf.get_variable("embedding", [vocab_size, size], dtype=tf.float32)
		...
		test_state_out = tf.reshape(tf.stack(axis=0, values=test_output_state), 
			[config.num_layers, 2, 1, size], name="test_state_out")
