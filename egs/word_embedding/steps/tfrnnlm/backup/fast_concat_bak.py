...
def main(_):
	...
	skipgram_embeddings = tf.Variable(tf.random_normal(shape = [config.vocab_size, 100], stddev = 1.0), dtype= tf.float32)
	cbow_embeddings = tf.Variable(tf.random_normal(shape = [config.vocab_size, 100], stddev = 1.0), dtype = tf.float32)
	...
	with tf.Session() as sess:
		...
		skipgram_embeddings = tf.constant(skipgram_embed, shape = [config.vocab_size, 100], dtype = tf.float32)
		cbow_embeddings = tf.constant(cbow_embed, shape = [config.vocab_size, 100], dtype = tf.float32)
