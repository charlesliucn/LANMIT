...
# the following script uses TensorFlow. You could use tools/extras/install_tensorflow_py.sh to install it
  $cuda_cmd $dir/train_fast_skipgram.log utils/parallel/limit_num_gpus.sh \
    python steps/tfrnnlm/fast_skipgram.py --data-path=$dir \
    --save-path=$dir/rnnlm --vocab-path=$dir/wordlist.rnn.final \
    --hidden-size=500 --num-layers=1 --batch-size=64 \
    --keep-prob=0.8 --max-epoch=30
fi
...
if [ $stage -le 3 ]; then
  ...
  steps/lmrescore_rnnlm_lat.sh \
    ...
    ${data_dir}/lang_200k $dir \
    ${data_dir}/dev10h_bnf ${decode_dir} \
    ${decode_dir}_${tmp_dir}
fi
