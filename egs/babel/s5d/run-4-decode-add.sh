...
if [ ! -f ${decode}/.done ]; then
  ...
  echo ---------------------------------------------------------------------
  utils/mkgraph.sh \
    ${data_dir}/lang_${add_countk}k ${exp_dir}/tri5 ${exp_dir}/tri5/graph_${add_countk}k | \
    tee ${exp_dir}/tri5/mkgraph_${add_countk}k.log
  ...
  steps/decode_fmllr_extra.sh --skip-scoring false --beam 10 --lattice-beam 4\
    ...
    ${exp_dir}/tri5/graph_${add_countk}k ${dataset_dir} ${decode} | tee ${decode}/decode.log
  touch ${decode}/.done
fi
...
if [ -f ${exp_dir}/sgmm5/.done ]; then
  ...
  if [ ! -f $decode/.done ]; then
    ...
    echo ---------------------------------------------------------------------
    utils/mkgraph.sh \
      ${data_dir}/lang_${add_countk}k ${exp_dir}/sgmm5 ${exp_dir}/sgmm5/graph_${add_countk}k | \
      tee ${exp_dir}/sgmm5/mkgraph_${add_countk}k.log
    ...
    steps/decode_sgmm2.sh --skip-scoring false --use-fmllr true --nj $my_nj \
      ...
      ${exp_dir}/sgmm5/graph_${add_countk}k ${dataset_dir} $decode | tee $decode/decode.log
    touch $decode/.done
  ...
  for iter in 1 2 3; do
    ...
    if [ -x ${exp_dir}/sgmm5_mmi_b0.1 ] && [ ! -f $decode/.done ]; then
      ...
      steps/decode_sgmm2_rescore.sh  --skip-scoring false \
        ...
        ${data_dir}/lang_${add_countk}k ${dataset_dir} \
        ${exp_dir}/sgmm5/decode_fmllr_${dataset_id} $decode | tee ${decode}/decode.log
